...
namespace Сycles
{
    class InputDataWithCheck
    {
        ...
        static public int InputIntegerWithValidation(string message, int left = int.MinValue, int right = int.MinValue)
        {
            bool ok;
            int value;
            do
        ...
        static public double InputDoubleWithValidation(string message, double left = double.MinValue, double right = long.MinValue)
        {
            bool ok;
            double value;
            do
        ...
        // Ввод числа long с проверкой правильности ввода и принадлежности диапазону
        static public long InputLongWithValidation(string message, long left = long.MinValue, long right = long.MaxValue)
        {
            bool ok;
            long value;
            do
            {
                Console.Write(message);
                ok = long.TryParse(Console.ReadLine(), out value);
                if (ok && (value < left || value > right))
                {
                    ok = false;
                }
